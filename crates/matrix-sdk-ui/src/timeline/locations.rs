//! This module handles rendering of MSC3489 location streams in the timeline.
...
use ruma::{
    events::location::{
        ...
        start::LocationKind,
        unstable_response::UnstableLocationResponseEventContent,
        unstable_start::{
            NewUnstableLocationStartEventContent, NewUnstableLocationStartEventContentWithoutRelation,
            UnstableLocationStartContentBlock,
        },
...
/// Holds the state of a location.
///
/// This struct should be created for each location start event handled and then
/// updated whenever handling any location response or location end event that relates
/// to the same location start event.
#[derive(Clone, Debug)]
pub struct LocationState {
    pub(super) start_event_content: NewUnstableLocationStartEventContent,
...
/// Data structure keeping track of
/// (Look into iOS SDK for that)
/// Room
/// - who is sharing the location
/// - which ro
...
impl LocationState {
    pub(super) fn new(content: NewUnstableLocationStartEventContent) -> Self {
        Self {
    ...
    pub(super) fn edit(
        &self,
        replacement: &NewUnstableLocationStartEventContentWithoutRelation,
    ) -> Result<Self, ()> {
    ...
    pub(super) fn add_response(
        ...
        content: &UnstableLocationResponseEventContent,
    ) -> Self {
        let mut clone = self.clone();
    ...
    /// Marks the location as ended.
    ///
    /// If the location has already ended, returns `Err(())`.
    ...
    pub fn results(&self) -> LocationResult {
        let results = compile_unstable_poll_results(
            &self.start_event_content.poll_start,
            self.response_data.iter().map(|response_data| LocationResponseData {
        ...
        LocationResult {
            ...
            answers: self
                ...
                .answers
                .iter()
                .map(|i| LocationResultAnswer { id: i.id.clone(), text: i.text.clone() })
                .collect(),
            votes: results
                .iter()
...
impl From<LocationState> for NewUnstableLocationStartEventContent {
    fn from(value: LocationState) -> Self {
        let content = UnstableLocationStartContentBlock::new(
        ...
        if let Some(text) = value.fallback_text() {
            NewUnstableLocationStartEventContent::plain_text(text, content)
        } else {
            NewUnstableLocationStartEventContent::new(content)
        }
    }
}

/// Acts as a cache for location response and location end events handled before their
/// start event has been handled.
#[derive(Clone, Debug, Default)]
pub(super) struct LocationPendingEvents {
...
impl LocationPendingEvents {
    ...
    pub(super) fn apply(&mut self, start_event_id: &EventId, poll_state: &mut LocationState) {
...
pub struct LocationResult {
    ...
    pub answers: Vec<LocationResultAnswer>,
...
#[derive(Debug)]
pub struct LocationResultAnswer {
    pub id: String,
    pub text: String,
}
